...
本章题库只覆盖第 4 章“软件测试方法”。题目按小部分拆开，每个小部分固定包含 ==2 道问答题 + 5 道选择题==。选择题统一提供 A/B/C/D 四个选项、正确答案和解析，方便中文考试前直接背。
...
- 本题库不再做中英逐句对照；只有 PPT 中原本使用的术语和缩写会保留，例如 Ad-hoc、ALAC、Pairwise、DD 路径、MC/DC、p-use、c-use。
...
- A. 每个条件都能独立影响判定结果
- B. 每个判定结果至少变化一次即可
- C. 每个条件取过 true 和 false 即可，不要求独立影响判定
- D. 每条可执行路径至少执行一次
...
本节按期末大题风格自拟 4 道综合题。重点不是只背结论，而是背 ==题目识别 -> 方法选择 -> 表格/图形输出 -> 用例设计 -> 解析说明== 这一套答题过程。

### 综合大题 14-1：黑盒测试综合设计：等价类 + 边界值 + 判定表
...
- 若任一输入不在 `0-100`，输出“输入非法”。
- 若输入均合法，但 `C < 40`，输出“期末不达标，不通过”。
- 若输入均合法，且 `C >= 40`，计算总评 `S = 0.2A + 0.3B + 0.5C`。
- 若 `S >= 60`，输出“通过”；否则输出“不通过”。
...
| 对象 | 有效等价类 | 无效等价类 |
| --- | --- | --- |
| 平时成绩 A | `0 <= A <= 100` | `A < 0`；`A > 100` |
| 实验成绩 B | `0 <= B <= 100` | `B < 0`；`B > 100` |
| 期末成绩 C | `0 <= C <= 100` | `C < 0`；`C > 100` |
| 输出规则 | `C < 40`；`C >= 40 且 S >= 60`；`C >= 40 且 S < 60` | 任一输入非法 |

**2. 边界值分析**

每个成绩输入的关键边界都是 `0` 和 `100`，因此每个变量都应重点考虑：

| 变量 | 下边界附近 | 上边界附近 |
| --- | --- | --- |
| A / B / C | `-1, 0, 1` | `99, 100, 101` |
...
| --- | --- |
| 期末成绩门槛 | `C = 39, 40` |
| 总评通过门槛 | `S = 59.5 或 59` 附近；`S = 60`；`S > 60` |
...
| 用例 ID | A | B | C | S 计算 | 预期输出 | 覆盖点 |
| --- | --- | --- | --- | --- | --- | --- |
| TC1 | 80 | 80 | 80 | `16+24+40=80` | 通过 | 正常有效类，R3 |
| TC2 | -1 | 80 | 80 | 不计算 | 输入非法 | A 下界外，无效等价类，R1 |
| TC3 | 101 | 80 | 80 | 不计算 | 输入非法 | A 上界外，无效等价类，R1 |
| TC4 | 80 | -1 | 80 | 不计算 | 输入非法 | B 下界外，无效等价类，R1 |
| TC5 | 80 | 80 | 101 | 不计算 | 输入非法 | C 上界外，无效等价类，R1 |
| TC6 | 0 | 0 | 100 | `0+0+50=50` | 不通过 | 合法边界 `0`，总评不足，R4 |
| TC7 | 100 | 100 | 100 | `20+30+50=100` | 通过 | 合法边界 `100`，R3 |
| TC8 | 100 | 100 | 39 | `20+30+19.5=69.5` | 期末不达标，不通过 | `C=39`，优先触发 R2 |
| TC9 | 100 | 100 | 40 | `20+30+20=70` | 通过 | `C=40`，期末门槛边界，R3 |
| TC10 | 50 | 50 | 70 | `10+15+35=60` | 通过 | `S=60`，总评通过边界，R3 |
| TC11 | 50 | 50 | 69 | `10+15+34.5=59.5` | 不通过 | `S<60` 边界附近，R4 |

#### 解析

这道题综合考察 ==等价类划分、边界值分析和判定表==。答题时最容易丢分的地方有三个：

1. 只写 `A/B/C` 的合法范围，却忘记每个输入都有 `<0` 和 `>100` 两类无效等价类。
2. 只测 `0` 和 `100`，却忘记非法边界 `-1` 和 `101`。
3. 看到 `S >= 60` 就直接判断通过，忘记 `C < 40` 具有更高优先级。TC8 的作用就是专门卡这个易错点。

考试中如果时间不够，至少要写出：等价类表、关键边界、判定表、测试用例表。用例表必须包含 ==输入 + 预期输出 + 覆盖点==，只列输入值一般不够完整。

### 综合大题 14-2：因果图与判定表综合题
...
- `A1`：免运费。条件：`C1 OR C2`。
- `A2`：使用优惠券立减 20 元。条件：`C3 AND NOT C4`。
- `A3`：赠送首单礼包。条件：`C4 AND NOT C3`。
- `A4`：进入人工确认优惠。条件：`C3 AND C4`，因为优惠券和首单礼包不能自动叠加。

约束条件：`C1` 和 `C4` 不能同时为真，即会员不能再被当作新用户首单。
...
flowchart LR
  ...
  C3["C3 有效优惠券"] --> AND1{"AND"}
  NC4["NOT C4"] --> AND1
  AND1 --> A2["A2 立减 20 元"]

  C4["C4 新用户首单"] --> AND2{"AND"}
  NC3["NOT C3"] --> AND2
  AND2 --> A3["A3 首单礼包"]
...
| 条件/动作 | R1 非法组合 | R2 优惠冲突 | R3 会员免邮 | R4 金额免邮 | R5 优惠券 | R6 首单 | R7 无优惠 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| C1 是会员 | Y | N | Y | N | N | N | N |
| C2 金额 `>=200` | - | - | N | Y | N | N | N |
| C3 有效优惠券 | - | Y | N | N | Y | N | N |
| C4 新用户首单 | Y | Y | N | N | N | Y | N |
| A1 免运费 | - | 视 C2 而定 | X | X |  |  |  |
| A2 立减 20 |  |  |  |  | X |  |  |
| A3 首单礼包 |  |  |  |  |  | X |  |
| A4 人工确认 |  | X |  |  |  |  |  |
| 处理说明 | 非法输入/约束冲突 | 优惠冲突 | 正常会员规则 | 正常金额规则 | 正常券规则 | 正常首单规则 | 无优惠 |

**4. 测试用例表**

| 用例 ID | 会员 C1 | 金额 | 优惠券 C3 | 首单 C4 | 预期动作 | 覆盖规则 |
| --- | --- | --- | --- | --- | --- | --- |
| TC1 | Y | 100 | N | Y | 非法输入/约束冲突 | R1，C1 与 C4 互斥 |
| TC2 | N | 100 | Y | Y | 人工确认优惠 | R2，C3 与 C4 冲突 |
| TC3 | Y | 150 | N | N | 免运费 | R3，会员免邮 |
| TC4 | N | 250 | N | N | 免运费 | R4，金额免邮 |
| TC5 | N | 100 | Y | N | 立减 20 元 | R5，优惠券正常使用 |
| TC6 | N | 100 | N | Y | 首单礼包 | R6，首单正常使用 |
| TC7 | N | 100 | N | N | 无优惠 | R7，默认规则 |

#### 解析

这道题考的是 ==因果图 -> 判定表 -> 测试用例== 的转换。因果图适合处理“多个输入条件通过逻辑关系决定多个输出动作”的题。判定表适合把这些逻辑关系落成可执行的测试规则。

答题时要注意：

- 有 `OR`、`AND`、`NOT` 时，先写清原因和结果，不要直接乱列用例。
- 有约束条件时必须单独写，例如本题的 `C1` 与 `C4` 互斥。
- 判定表不一定要列出 `2^4=16` 个组合。考试中可以合并无关项，用 `-` 表示对该规则无影响。
- 测试用例要覆盖正常规则，也要覆盖冲突规则和默认规则。
...
1. 标出判定和基本条件。
2. 设计一组测试用例，使其满足 ==判定-条件覆盖==。
3. 说明这组用例是否一定满足条件组合覆盖。
4. 写出每个测试用例的预期输出。
...
| 用例 ID | x | y | z | C1 | C2 | C3 | D1 | C4 | C5 | D2 | 预期输出 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TC1 | 6 | 6 | 0 | T | T | F | T | T | T | T | A |
| TC2 | 1 | 1 | 0 | T | T | F | T | F | T | F | B |
| TC3 | -1 | 5 | 0 | F | T | F | F | - | - | - | C |
| TC4 | 5 | -1 | 1 | T | F | T | T | F | T | F | B |
| TC5 | 6 | 6 | 2 | T | T | F | T | T | F | F | B |

覆盖检查：

| 覆盖目标 | 是否满足 | 说明 |
| --- | --- | --- |
| D1 取 T/F | 满足 | TC1/TC2/TC4/TC5 使 D1=T，TC3 使 D1=F |
| D2 取 T/F | 满足 | TC1 使 D2=T，TC2/TC4/TC5 使 D2=F |
| C1 取 T/F | 满足 | TC1 为 T，TC3 为 F |
| C2 取 T/F | 满足 | TC1 为 T，TC4 为 F |
| C3 取 T/F | 满足 | TC4 为 T，TC1 为 F |
| C4 取 T/F | 满足 | TC1 为 T，TC2/TC4 为 F |
| C5 取 T/F | 满足 | TC1 为 T，TC5 为 F |
...
1. 每个判定至少取过一次 T 和 F。
2. 每个基本条件至少取过一次 T 和 F。

但是条件组合覆盖要求每个判定内部的条件组合都覆盖。例如：

- D1 有 3 个基本条件，理论上有 `2^3 = 8` 种组合。
- D2 有 2 个基本条件，理论上有 `2^2 = 4` 种组合。

上表只覆盖了部分组合，因此满足判定-条件覆盖，但不能说一定满足条件组合覆盖。

#### 解析

这道题考察 ==判定覆盖、条件覆盖、判定-条件覆盖、条件组合覆盖== 的区别。最常见的失分点是把“每个条件都取过 T/F”误认为“每个条件组合都覆盖了”。这两个不是一回事。

答题技巧：

- 先拆判定，再拆基本条件。
- 表格中写清每个条件的 T/F。
- 如果某个用例没有进入内层判定 D2，D2、C4、C5 可以记为 `-`，表示未执行。
- 判定-条件覆盖强于单纯判定覆盖或单纯条件覆盖，但弱于条件组合覆盖。

### 综合大题 14-4：基本路径 + 数据流 + 变异测试综合题
...
1. 画出控制流图或用节点文字表示控制流图。
2. 计算圈复杂度 `V(G)`。
3. 列出一组基本路径。
4. 为每条基本路径设计测试用例并写预期输出。
5. 指出变量 `price` 的定义和使用关系。
6. 给出至少 2 个变异体，并说明如何击杀。
...
```mermaid
flowchart TD
  N1["1 price = 0"] --> N2{"2 type == vip?"}
  N2 -->|T| N3["3 price = 80"]
  N2 -->|F| N4["4 price = 100"]
  N3 --> N5{"5 days > 7?"}
  N4 --> N5
  N5 -->|T| N6["6 price = price * 0.9"]
  N5 -->|F| N7{"7 late?"}
  N6 --> N7
  N7 -->|T| N8["8 price = price + 20"]
  N7 -->|F| N9["9 return price"]
  N8 --> N9
```
...
也可以用判定节点数计算：有 3 个判定节点，因此：

`V(G) = P + 1 = 3 + 1 = 4`

所以至少需要 4 条独立路径组成基本路径集。

**3. 基本路径与测试用例**

| 路径 | 节点序列 | 测试输入 | 预期输出 | 覆盖点 |
| --- | --- | --- | --- | --- |
| P1 | `1-2T-3-5T-6-7T-8-9` | `type="vip", days=8, late=true` | `80*0.9+20=92` | VIP、超过 7 天、逾期 |
| P2 | `1-2T-3-5F-7F-9` | `type="vip", days=3, late=false` | `80` | VIP、不超过 7 天、不逾期 |
| P3 | `1-2F-4-5T-6-7F-9` | `type="normal", days=10, late=false` | `100*0.9=90` | 普通用户、超过 7 天、不逾期 |
| P4 | `1-2F-4-5F-7T-8-9` | `type="normal", days=2, late=true` | `100+20=120` | 普通用户、不超过 7 天、逾期 |

**4. `price` 的定义和使用关系**

| 编号 | 语句 | 类型 | 说明 |
| --- | --- | --- | --- |
| d0 | `let price = 0` | 定义 | 初始定义，随后一定被 `80` 或 `100` 覆盖 |
| d1 | `price = 80` | 定义 | VIP 基础价格 |
| d2 | `price = 100` | 定义 | 普通用户基础价格 |
| u1/d3 | `price = price * 0.9` | 使用 + 定义 | 使用 d1 或 d2，再定义折扣后价格 |
| u2/d4 | `price = price + 20` | 使用 + 定义 | 使用当前价格，再定义逾期后价格 |
| u3 | `return price` | 使用 | 返回最终价格 |

典型定义-使用路径：

| 定义-使用对 | 示例路径 |
| --- | --- |
| `d1 -> u1` | VIP 且 `days > 7`，如 P1 |
| `d1 -> u3` | VIP 且不打折不逾期，如 P2 |
| `d2 -> u1` | 普通用户且 `days > 7`，如 P3 |
| `d2 -> u2` | 普通用户且逾期，如 P4 |
| `d3 -> u2` | 打折后又逾期，如 P1 |
| `d4 -> u3` | 逾期加价后返回，如 P1/P4 |

**5. 变异体与击杀用例**

| 变异体 | 变异内容 | 击杀用例 | 原程序输出 | 变异程序输出 | 是否击杀 |
| --- | --- | --- | --- | --- | --- |
| M1 | `days > 7` 改成 `days >= 7` | `type="normal", days=7, late=false` | `100` | `90` | 是 |
| M2 | `type === "vip"` 改成 `type !== "vip"` | `type="vip", days=2, late=false` | `80` | `100` | 是 |
| M3 | `price + 20` 改成 `price + 10` | `type="normal", days=2, late=true` | `120` | `110` | 是 |

#### 解析

这道题把 ==控制流图、圈复杂度、基本路径、数据流测试和变异测试== 放在一起考。完整大题的答题顺序建议是：

1. 先画控制流图或至少列节点和边。
2. 再算 `V(G)`，同时用 `E-N+2` 和 `P+1` 互相验证。
3. 列出与 `V(G)` 数量一致的基本路径。
4. 每条路径配一个输入，并计算预期输出。
5. 数据流题要写清定义和使用，尤其是 `price = price * 0.9` 这种“右边使用，左边重新定义”的语句。
6. 变异测试题必须写出变异体、击杀用例、原输出和变异输出。只写“能击杀”不给输出差异，答案不够完整。
