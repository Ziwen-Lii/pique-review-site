# 软件测试作业题详解：等价类、边界值、因果图、黑盒、覆盖与基本路径

本章来自 `作业.docx`，把 6 个作业题全部整理成可直接提交/复习的答案。重点不是只给测试用例，而是写清楚：==为什么这样划分、覆盖了什么、预期输出是什么、如果考试/作业要求解释该怎么写==。
...
| 约束 | 含义 |
| --- | --- |
| C1 与 C2 互斥 | 一次只投入一种金额 |
| C3 与 C4 互斥 | 一次只选择一种饮料 |
| C5 与 C6 互斥 | 要么有零钱，要么没零钱 |
...
说明：图里“饮料类型”实际由 C3/C4 决定：按橙汁出橙汁，按啤酒出啤酒。

### 3.4 判定表

| 规则 | 金额 | 按钮 | 零钱状态 | 红灯 | 饮料 | 退款 | 说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| R1 | 5 | 橙汁 | 有零钱 | 灭 | 橙汁 | 无 | 刚好付款 |
| R2 | 5 | 啤酒 | 有零钱 | 灭 | 啤酒 | 无 | 刚好付款 |
| R3 | 10 | 橙汁 | 有零钱 | 灭 | 橙汁 | 退 5 元 | 需要找零 |
| R4 | 10 | 啤酒 | 有零钱 | 灭 | 啤酒 | 退 5 元 | 需要找零 |
| R5 | 5 | 橙汁 | 无零钱 | 亮 | 橙汁 | 无 | 5 元不需要找零 |
| R6 | 5 | 啤酒 | 无零钱 | 亮 | 啤酒 | 无 | 5 元不需要找零 |
| R7 | 10 | 橙汁 | 无零钱 | 亮 | 不出 | 退 10 元 | 无法找零 |
| R8 | 10 | 啤酒 | 无零钱 | 亮 | 不出 | 退 10 元 | 无法找零 |

### 3.5 测试用例表

| ID | 投入金额 | 选择按钮 | 零钱状态 | 预期结果 | 覆盖规则 |
| --- | --- | --- | --- | --- | --- |
| TC1 | 5 | 橙汁 | 有零钱 | 红灯灭，出橙汁，不退款 | R1 |
| TC2 | 5 | 啤酒 | 有零钱 | 红灯灭，出啤酒，不退款 | R2 |
| TC3 | 10 | 橙汁 | 有零钱 | 红灯灭，出橙汁，退 5 元 | R3 |
| TC4 | 10 | 啤酒 | 有零钱 | 红灯灭，出啤酒，退 5 元 | R4 |
| TC5 | 5 | 橙汁 | 无零钱 | 红灯亮，出橙汁，不退款 | R5 |
| TC6 | 5 | 啤酒 | 无零钱 | 红灯亮，出啤酒，不退款 | R6 |
| TC7 | 10 | 橙汁 | 无零钱 | 红灯亮，不出饮料，退 10 元 | R7 |
| TC8 | 10 | 啤酒 | 无零钱 | 红灯亮，不出饮料，退 10 元 | R8 |

### 3.6 解析

这题的核心不是“金额有两个、按钮有两个、零钱有两个，所以凑 8 个用例”这么简单，而是要解释规则：

- 有零钱时，10 元可以购买并退 5 元。
- 无零钱时，10 元不能购买，必须退 10 元。
- 5 元刚好购买，不受找零能力影响。
- 红灯只反映零钱状态：有零钱灭，无零钱亮。
...
```mermaid
flowchart TD
  A["1 start / i_temp=0"] --> B{"2 i_count > 0 ?"}
  B -->|F| H["8 return i_temp"]
  B -->|T| C{"3 i_flag == 0 ?"}
  C -->|T| D["4 i_temp = i_count + 100<br/>break"]
  D --> H
  C -->|F| E{"5 i_flag == 1 ?"}
  E -->|T| F["6 i_temp = i_temp + 10"]
  E -->|F| G["7 i_temp = i_temp + 20"]
  F --> I["9 i_count--"]
  G --> I
  I --> B
```
...
| 路径 | 走法 | 说明 |
| --- | --- | --- |
| P1 | `1 -> 2(F) -> 8` | `while` 不进入，直接返回 |
| P2 | `1 -> 2(T) -> 3(T) -> 4 -> 8` | `i_flag=0`，执行 `break` |
| P3 | `1 -> 2(T) -> 3(F) -> 5(T) -> 6 -> 9 -> 2(F) -> 8` | `i_flag=1`，循环一次后退出 |
| P4 | `1 -> 2(T) -> 3(F) -> 5(F) -> 7 -> 9 -> 2(F) -> 8` | `i_flag` 既不是 0 也不是 1，循环一次后退出 |
